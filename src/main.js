...
async function runDetection(input, output) {
  ...
  if (output) {
    ...
    if (options.debug) log.state('Saved processed image');
  }
  if (options.debug) log.state(`done: model:${options.modelPath} time:${(t1 - t0) / 1000n / 1000n} input:${input} output:${output} objects:`, res.parts?.length);
  return res;
}

// main function
let tfReady = false;
async function detect(inputPath, outputPath, debug = false) {
  options.debug = debug;
  if (options.debug) log.header();
  if (!tfReady) {
  ...
  return runDetection(inputPath, outputPath);
}
